function Home({ go, add }) {
  ...
  return (
    <React.Fragment>
      <section className="hero clr">
        <div className="clr-strip" aria-hidden="true"><div className="clr-track">{Array.from({ length: 12 }).map((_, i) => <span key={i}>{["Summer Clearance", "Sale", "Limited Time"][i % 3]}<i>★</i></span>)}</div></div>
        <div className="clr-in">
          <div className="clr-copy">
            <h1 className="h-title">Summer Clearance</h1>
            <p className="hero-sub">Shop our best deals</p>
            <button className="btn-wg" onClick={() => go("#/c/clearance")}>Shop Clearance</button>
          </div>
          <div className="clr-badge">
            <span className="clr-up">Up to</span>
            <span className="clr-num">75<i>%</i></span>
            <span className="clr-off">Off</span>
          </div>
        </div>
        <div className="clr-strip clr-strip-b" aria-hidden="true"><div className="clr-track">{Array.from({ length: 12 }).map((_, i) => <span key={i}>{["Summer Clearance", "Sale", "Limited Time"][(i + 1) % 3]}<i>★</i></span>)}</div></div>
      </section>

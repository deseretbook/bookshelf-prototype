...
(function () {
  ...
  window.DB_DATA = {
    ...
    inCat: (id) => (id === "books_gospel-voices" ? GV_ORDER.map((x) => P.find((p) => p.id === x)).filter(Boolean) : id === "new-trending" ? P.slice() : id === "clearance" ? P.filter((p, i) => i % 2 === 0) : P.filter((p) => p.cat === id || p.cat.indexOf(id) === 0)),

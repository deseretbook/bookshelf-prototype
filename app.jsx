...
function App() {
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => setPromo(true), 1200);
  ...
  return (
    <div className="phone">
      ...
      <div className="ovh">
        ...
        <PromoDialog open={promo} onClose={closePromo} />
      </div>
      <TweaksPanel>
        <TweakSection label="Free-shipping bar" />
        <TweakRadio label="Style" value={t.shipStyle} options={["bar", "segments"]} onChange={(v) => setTweak("shipStyle", v)} />
        <TweakColor label="Accent" value={t.shipAccent} options={["#0f8079", "#ac761e", "#3a3633"]} onChange={(v) => setTweak("shipAccent", v)} />
        <TweakSlider label="Thickness" value={t.shipHeight} min={4} max={14} unit="px" onChange={(v) => setTweak("shipHeight", v)} />
        <TweakSlider label="Threshold ($)" value={t.shipThreshold} min={25} max={99} onChange={(v) => setTweak("shipThreshold", v)} />
        <TweakToggle label="Show goal amount" value={t.shipGoalMark} onChange={(v) => setTweak("shipGoalMark", v)} />
      </TweaksPanel>
    </div>
  );
}

...
function Cover({ p }) {
  ...
  if (state === "bad" || !src) return <span className="ph">{p.t}</span>;
  return (
    <React.Fragment>
      {state === "load" ? <span className="ph" style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", padding: "0 10px" }}>{p.t}</span> : null}
      <img src={src} alt={p.t} onLoad={() => setState("ok")} onError={() => setState("bad")} style={{ opacity: state === "ok" ? 1 : 0 }} />
    </React.Fragment>
  );
}
...
function Wordmark() {
  return (
    <React.Fragment>
      <img className="wm-e" src="assets/emblem.svg" alt="" />
      <img className="wm-w" src="assets/wordmark.svg" alt="Deseret Book" />
    </React.Fragment>
  );
}

function Header({ count, onMenu, onSearch, onHome, onBag, onAccount, search, onCloseSearch, onOpenProduct, shipMin }) {
  return (
    <React.Fragment>
      ...
      <header className={"hdr" + (search ? " srch-on" : "")}>
        <div className="hdr-in">
          ...
          <div className="hdr-ico" style={{ justifyContent: "flex-end" }}>
            <button className="iconbtn" onClick={onAccount} aria-label="Account"><Ico name="user" size={20} /></button>
...
function HeaderSearch({ open, onOpen, onClose, onOpenProduct }) {
  ...
  return (
    <div className="srch" style={{ marginBottom: 4 }}>
      ...
      <div className={"srch-panel" + (open ? " on" : "")} aria-hidden={!open}>
        {term.length < 2 ? (
          ...
          <button key={p.id} className="srow" onClick={() => { onOpenProduct(p.id); onClose(); }}>
            <span className="th"><img src={thumb(p.img[0])} alt="" /></span>
            <span>
  ...
  return (
    <section className="news">
      <div style={{ display: "flex", flexDirection: "column", gap: 4, alignSelf: "stretch" }}>
        <h2>Inspiration Made Easy</h2>
        <p>Sign up for discounts, news and limited-time offers</p>
      </div>
      {done ? <p className="ok">You&rsquo;re on the list. We send one email a week.</p> : (
      ...
      {err ? <p className="ok" style={{ fontSize: 11 }}>{err}</p> : null}
    </section>
  );
}
...
function FootAcc({ title, items, onGo }) {
  ...
  return (
    <div className="acc">
      ...
      {open ? <div className="acc-b">{items.map((it) => <a key={it} href="#/page" onClick={(e) => { e.preventDefault(); onGo(it); }}>{it}</a>)}</div> : null}
    </div>
  );
}
          ...
          <div className="bagd-b">
            ...
            {lines.map((x, i) => (
              <div className="line" key={x.l.key || i}>
                <button className="th" onClick={() => { onClose(); go("#/p/" + x.p.id); }}><img src={thumb(x.p.img[0])} alt="" /></button>
                <div className="info">
...
Object.assign(window, {
  DS, Ico, StatusBar, Stars, Cover, ProductCard, useRailDots, Dots, Wordmark, Header, Drawer,
